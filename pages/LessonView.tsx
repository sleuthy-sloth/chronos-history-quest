  ...
  return (
    <div className="h-screen bg-stone-100 dark:bg-stone-950 flex flex-col font-serif overflow-hidden">
        ...
        <div className="flex items-center gap-4 p-4 max-w-3xl mx-auto w-full z-10 bg-white/80 dark:bg-stone-900/80 backdrop-blur border-b border-stone-200 dark:border-stone-800">
            <button onClick={onExit} className="text-stone-400 hover:text-stone-600 text-xl font-bold p-2 rounded-lg hover:bg-stone-200/50">✕</button>
            ...
            {(activity.type === ActivityType.SORTING) && (
                 <div className="space-y-4">
                     <div className="bg-stone-100 dark:bg-stone-900 p-4 rounded-2xl border-2 border-stone-300 dark:border-stone-700 border-dashed min-h-[120px] flex flex-col gap-2">
                        {sortedItems.length === 0 && <p className="text-stone-400 italic text-center text-sm font-mono uppercase mt-8">Sequence Required</p>}
                        {sortedItems.map(item => (
                            <div key={item} className={`p-4 bg-white dark:bg-stone-800 border border-stone-400 rounded-xl shadow-sm font-bold flex gap-3 items-center animate-fade-in-up`}>
                                <span className={`w-6 h-6 rounded-full ${theme.primary} text-white flex items-center justify-center text-xs shadow-sm`}>✓</span>
                                {item}
                            </div>
                        ))}
                     </div>
                     <div className="grid gap-3">
                         {shuffledSortingPool.map(item => (
                             <button 
                                ...
                                className="p-4 bg-white dark:bg-stone-800 border-b-4 border-stone-300 dark:border-stone-700 font-bold text-stone-700 dark:text-stone-300 hover:bg-stone-50 active:border-b-0 active:translate-y-1 text-left rounded-xl transition-all"
                             >
                                 {item}
                             </button>
                         ))}
